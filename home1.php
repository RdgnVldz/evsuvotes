...
<style>
        /* Add some basic styling for layout and readability */
        body {
        ...
        label {
    display: block;
    font-size: 30px; /* Adjust the font size to your preference, e.g., 20px */
    margin: 5px 0;
                        ...
                        <div class="box-body">
                            <div class="tab-content">
                                ...
                                include 'includes/slugify.php';
                                    ...
                                    // Query to get positions and candidates, ordered by priority
                                    $sql = "SELECT p.id AS position_id, p.description, c.id AS candidate_id, c.firstname, c.lastname, c.partylist, c.photo, p.max_vote 
                                            FROM positions p
                                            LEFT JOIN candidates c ON p.id = c.position_id
                                            ORDER BY p.priority, c.lastname";
                                    ...
                                    if ($result) {
                                        ...
                                        while ($row = $result->fetch_assoc()) {
                                            if ($currentPosition !== $row['position_id']) {
                                                ...
                                                echo "<h2><strong>" . $positionsData[$positionCount]['description'] . "</strong></h2>";
                                                echo "<button class='reset-button pull-right btn btn-success' type='button' onclick='resetVotes(this)'>Reset</button>";                                                
                                                // Display the message based on the max_vote count from the positions table
                                                echo "<p>";
                                                if ($positionsData[$positionCount]['max_vote'] === 1) {
                                                    echo "Select only one candidate.";
                                                } else {
                                                    echo "<span style='color: red;'><b>Note!</b> </span>You may select up to {$positionsData[$positionCount]['max_vote']} candidates.";
                                                }
                                                echo "</p>";
                                                $positionCount++;
                                            }
                                            ...
                                            if ($row['max_vote'] > 1) {
                                                // Display larger checkboxes with padding
                                                echo "<label for='candidate{$row['candidate_id']}' style='padding: 5px;'><input type='checkbox' style='transform: scale(1.5);' name='vote[{$row['position_id']}][]' value='{$row['candidate_id']}' id='candidate{$row['candidate_id']}'>";
                                            
                                                if (!empty($row['photo'])) {
                                                    // Display the candidate's photo if it exists with margin and padding
                                                    echo "<img src='" . htmlspecialchars($row['photo']) . "' width='100' height='100' style='margin-right: 20px; padding: 20px;'>"; // Adjust the margin and padding values as needed
                                                } else {
                                                    // Display the default picture if no photo exists with margin and padding
                                                    echo "<img src='images/profile.jpg' width='100' height='100' style='margin-right: 20px; padding: 20px;'>"; // Adjust the margin and padding values as needed
                                                }
                                            
                                                echo htmlspecialchars($row['firstname']) . " " . htmlspecialchars($row['lastname']) . "</label><br>";
                                            } else {
                                                // Display larger radio buttons with padding
                                                echo "<label for='candidate{$row['candidate_id']}' style='padding: 5px;'><input type='radio' style='transform: scale(1.5);' name='vote[{$row['position_id']}]' value='{$row['candidate_id']}' id='candidate{$row['candidate_id']}'>";
                                    ...
                                    <div class="navigation-buttons">
                                        <input class="pull-left" type="button" id="previousButton" value="Previous">
                                        <input class="pull-right" type="button" id="nextButton" value="Next">
                                        <input class="pull-right" type="submit" id="voteButton" value="Vote">
                                    </div>
                                </form>
                            </div>
                        </div>
